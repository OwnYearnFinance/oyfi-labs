...
contract('Unipool', function ([_, wallet1, wallet2, wallet3, wallet4]) {
    describe('Unipool', async function () {
        ...
        it('Two stakers with the same stakes wait 1 w', async function () {
            // 1000 OYFI per week for 3 weeks
        ...
        it('One staker on 2 durations with gap', async function () {
            // 1000 OYFI per week for 1 weeks
            await this.pool.notifyRewardAmount(web3.utils.toWei('1000'), { from: wallet1 });
        ...
        it('Notify Reward Amount from mocked distribution to 10,00', async function () {
            // 1000 OYFI per week for 1 weeks
            await this.pool.notifyRewardAmount(web3.utils.toWei('1000'), { from: wallet1 });
            ...
            expect(await this.pool.rewardPerToken()).to.be.bignumber.almostEqualDiv1e18(web3.utils.toWei('250'));
            expect(await this.pool.earned(wallet1)).to.be.bignumber.almostEqualDiv1e18(web3.utils.toWei('250'));
            expect(await this.pool.earned(wallet2)).to.be.bignumber.almostEqualDiv1e18(web3.utils.toWei('750'));
        });
    });
});
